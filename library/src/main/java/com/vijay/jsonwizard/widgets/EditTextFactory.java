...
import com.vijay.jsonwizard.validators.edittext.CPFValidator;
import com.vijay.jsonwizard.validators.edittext.DateValidator;
...
public class EditTextFactory implements FormWidgetFactory {
    ...
    @Override
    public List<View> getViewsFromJson(String stepName, final Context context, JSONObject jsonObject, CommonListener listener) throws Exception {
...
//        renan.pinto HACK
        JSONObject cpfObject = jsonObject.optJSONObject("v_cpf");
        if(cpfObject != null) {
            editText.addValidator(new CPFValidator(cpfObject.getString("err")));
        }

        JSONObject dateObject = jsonObject.optJSONObject("v_date");
        if(dateObject != null) {
            final String dateFormat = dateObject.getString("format");
            boolean isBirthday = false;
            try {
                isBirthday = dateObject.getBoolean("birthday");
            }catch (Exception e){
                e.printStackTrace();
            }
            final DateValidator dValidator = new DateValidator(dateObject.getString("err"));
            dValidator.setBirthdate(isBirthday);
            dValidator.setOnDateSet(new DatePickerFragment.OnDateSet() {
                @Override
                public void onDateSet(int year, int month, int day) {
                    ...
                    long birthdate = c.getTimeInMillis();
                    String format = dateFormat == null?"yyyy-MM-dd":dateFormat;
                    String date =  new SimpleDateFormat(format).format(birthdate);
                    editText.setText(date);
                }
            });
            editText.addValidator(dValidator);
            editText.setFocusable(false);
            editText.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    dValidator.showDatePickerDialog(context);
                }
            });
        }
